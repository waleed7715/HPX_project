...
#         for c in 1 2; do
#             for ((r=1; r<=RUNS; r++)); do
#                 echo "Running: threads=$t size=$s num_chunks=$c run=$r"

#                 result=$("$BINARY" --hpx_threads="$t" --input_size="$s" --num_chunks="$c" 2>/dev/null)
...
BINARY="build/copy_if_hpx_clean"
    ...
    for s in 100000; do
        for c in 0 1 2; do
            for ((r=1; r<=RUNS; r++)); do
                ...
                result=$("$BINARY" --hpx:threads="$t" --input_size="$s" --num_chunks="$c" 2>/dev/null)
