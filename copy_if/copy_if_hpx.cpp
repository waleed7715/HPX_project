...
using alloc_t = hpx::compute::host::block_allocator<int>;
using numa_vec = hpx::compute::vector<int, alloc_t>;
...
int hpx_main(hpx::program_options::variables_map& vm)
{
    try {
        ...
        alloc_t alloc(numa_nodes);

        for (int i = 0; i < iterations; ++i) {
            std::string filename = "test_data_" + std::to_string(input_size) + ".bin";

            std::vector<int> raw = load_vector(filename);
            numa_vec source(raw.size(), alloc);
            hpx::copy(hpx::execution::par, raw.begin(), raw.end(), source.begin());
            ...
            auto run = [&](auto exec_policy) {
                numa_vec destination(input_size, alloc);
        ...
        std::cerr << "=== hpx::exception_list ===\n";
        for (auto const& e : el) {
            try {
                std::rethrow_exception(e);
            }
            catch (hpx::exception const& he) {
                std::cerr << "  what:     " << he.what() << "\n";
                std::cerr << "  location: " << hpx::get_error_file_name(he)
                          << ":" << hpx::get_error_line_number(he) << "\n";
                std::cerr << "  backtrace:\n" << hpx::get_error_backtrace(he) << "\n";
            }
            catch (std::exception const& se) {
                std::cerr << "  std::exception: " << se.what() << "\n";
            }
        }
    }
    catch (hpx::exception const& e) {
        std::cerr << "=== hpx::exception ===\n";
        std::cerr << "  what:     " << e.what() << "\n";
        std::cerr << "  location: " << hpx::get_error_file_name(e)
                  << ":" << hpx::get_error_line_number(e) << "\n";
        std::cerr << "  backtrace:\n" << hpx::get_error_backtrace(e) << "\n";
    }
