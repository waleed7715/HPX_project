    ...
    namespace detail {
        ...
        struct copy_if : public algorithm<copy_if<IterPair>, IterPair>
        {
                ...
                FwdIter3 dest, Pred&& pred, Proj&& proj /* = Proj()*/)
            {
                ...
                // No flags[] array: eliminates O(N) heap allocation, N flag
                // writes in F1, and N flag reads in F3. F3 re-evaluates pred
                // directly — same approach TBB uses. No zip_iterator: plain
                // FwdIter1 throughout reduces per-element iterator overhead.
                // No shared_ptr<atomic> counters outside ITT blocks.
                ...
                auto f1 = [pred = HPX_FORWARD(Pred, pred),
                    ...
                    static std::atomic<int> f1_counter{0};
                    ...
                    util::loop_n<std::decay_t<ExPolicy>>(part_begin, part_size,
                        [&pred, proj, &curr](FwdIter1 it) mutable -> void {
                            if (hpx::invoke(pred, hpx::invoke(proj, *it)))
                                ++curr;
                        });
                ...
                auto f3 = [dest, pred, proj](FwdIter1 part_begin,
                              std::size_t part_size,
                              std::size_t val) mutable {

                #if HPX_HAVE_ITTNOTIFY != 0 && !defined(HPX_HAVE_APEX)
                    static hpx::util::itt::domain f3_domain("copy_if");
                    static std::atomic<int> f3_counter{0};
                    ...
                    util::loop_n<std::decay_t<ExPolicy>>(part_begin, part_size,
                        [&dest, &pred, proj](FwdIter1 it) mutable -> void {
                            if (hpx::invoke(pred, hpx::invoke(proj, *it)))
                                *dest++ = *it;
                        });
                };

                auto f4 = [first, dest](std::vector<std::size_t>&& items,
